...
const ssLangOptions = Object.keys(languages).map((el) => {
  return { label: languages[el].name, value: el };
});

// Define custom fields schema
const strataProfileSchema = new SimpleSchema({
  ...
  'language': {
    ...
    autoform: {
      options: ssLangOptions,
    },
  },
});

var validationContext = strataProfileSchema.namedContext();

RocketChat.models.CustomUserFields.addSchema(strataProfileSchema);
